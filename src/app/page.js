...
export default function EditorPage() {
  ...
  if (!mounted) {
    return (
      <div className="flex flex-col h-screen bg-[var(--bg-base)] overflow-hidden" aria-hidden="true">
        {/* Topbar skeleton */}
        <div className="flex items-center gap-3 h-12 px-3 border-b border-[var(--sb-border)] bg-[var(--bg-topbar)] flex-shrink-0">
          <div className="w-8 h-8 rounded-md bg-[var(--tb-skeleton)] animate-pulse" />
          <div className="w-24 h-4 rounded bg-[var(--tb-skeleton)] animate-pulse" />
          <div className="flex-1" />
          <div className="w-16 h-8 rounded-md bg-[var(--tb-skeleton)] animate-pulse" />
          <div className="w-24 h-8 rounded-md bg-[var(--tb-skeleton)] animate-pulse" />
          <div className="w-16 h-8 rounded-md bg-[var(--tb-skeleton)] animate-pulse" />
          <div className="w-8 h-8 rounded-full bg-[var(--tb-skeleton)] animate-pulse" />
          <div className="w-8 h-8 rounded-full bg-[var(--tb-skeleton)] animate-pulse" />
        </div>
        {/* Body skeleton */}
        <div className="flex flex-1 min-h-0">
          {/* Sidebar skeleton */}
          <div className="w-60 border-r border-[var(--sb-border)] bg-[var(--sb-panel)] flex-shrink-0">
            <div className="flex items-center justify-between h-9 pl-3 pr-1.5 border-b border-[var(--sb-border)]">
              <div className="w-16 h-2.5 rounded bg-[var(--tb-skeleton)] animate-pulse" />
            </div>
            <div className="p-1">
              {[1,2,3,4].map((i) => (
                <div key={i} className="flex items-center gap-2 h-[26px] px-2 rounded">
                  <div className="w-3.5 h-3.5 rounded bg-[var(--tb-skeleton)] animate-pulse" />
                  <div className="h-2.5 rounded bg-[var(--tb-skeleton)] animate-pulse" style={{ width: `${50 + i * 12}%` }} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 flex min-h-0 min-w-0">
            {/* Editor skeleton */}
            <div className="flex-[0_0_65%] border-l border-[var(--sb-border)] bg-[var(--bg-panel)]" />
            {/* Resize handle */}
            <div className="w-1 flex-shrink-0 flex items-center justify-center">
              <div className="w-px h-8 rounded-full bg-[var(--sb-border)]" />
            </div>
            {/* IO skeleton */}
            <div className="flex-1 flex flex-col min-h-0 min-w-0">
              <div className="flex-[0_0_35%] border-l border-[var(--sb-border)] bg-[var(--bg-panel)]" />
              <div className="h-1 flex-shrink-0 flex items-center justify-center">
                <div className="w-8 h-px rounded-full bg-[var(--sb-border)]" />
              </div>
              <div className="flex-1 border-l border-[var(--sb-border)] bg-[var(--bg-panel)]" />
            </div>
          </div>
        </div>
      </div>
  ...
  const dialogClass = "bg-[var(--sb-panel)] border border-[var(--sb-border)] rounded-2xl p-6 w-80 shadow-2xl";
    ...
    <div className={`flex flex-col h-screen overflow-hidden bg-[var(--bg-base)] ${isDraggingH || isDraggingV ? "select-none cursor-col-resize" : ""}`}>
      ...
      >
        ...
        <main className="flex flex-1 min-h-0 min-w-0" ref={editorAreaRef} role="main">
          ...
          >
            {hasActiveFile ? (
              ...
              <div className="flex flex-1 items-center justify-center bg-[var(--bg-panel)]">
                <span className="text-[clamp(3rem,8vw,7rem)] font-black text-[var(--sb-text-muted)] tracking-tight select-none">
                  SUMORA
                </span>
              </div>
            )}
          ...
          <div
            className={`w-1 cursor-col-resize flex items-center justify-center group flex-shrink-0 hover:bg-[var(--border-subtle)] transition-colors ${isDraggingH ? "bg-[var(--sb-accent)]/20" : ""}`}
            onMouseDown={startHResize}
            title="Drag to resize editor"
            aria-hidden="true"
          >
            <div className={`w-px h-8 rounded-full transition-colors ${isDraggingH ? "bg-[var(--sb-accent)]" : "bg-[var(--sb-border)] group-hover:bg-[var(--sb-accent)]"}`} />
          </div>
      ...
      {logoutConfirmVisible && (
        <div className={overlayClass} onClick={(e) => { if (e.target === e.currentTarget) setLogoutConfirmVisible(false); }}>
          <div className={dialogClass}>
            ...
            <h3 className="text-sm font-bold text-[var(--text-primary)] text-center mb-2">Unsaved Changes</h3>
            <p className="text-xs text-[var(--text-secondary)] text-center mb-5">Do you want to save before logging out?</p>
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                <button className={`${btnBase} text-[var(--text-secondary)] bg-[var(--tb-btn-bg)] hover:bg-[var(--tb-btn-hover)]`} onClick={() => setLogoutConfirmVisible(false)}>
                  Cancel
                </button>
